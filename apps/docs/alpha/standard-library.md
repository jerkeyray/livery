...
| `frame` | primitive | supported | figure, component, canvas | `label?: string`<br>`subtitle?: string`<br>`layout?: identifier (row / column / grid / stack / overlay)`<br>`columns?: number`<br>`gap?: length`<br>`padding?: length`<br>`align?: string (start / center / end / stretch)`<br>`distribute?: string (start / center / end / between / around)`<br>`width?: length`<br>`height?: length`<br>`fill?: paint`<br>`stroke?: paint`<br>`strokeWidth?: length`<br>`opacity?: number`<br>`color?: paint`<br>`fontSize?: length`<br>`fontWeight?: number`<br>`radius?: length` |
